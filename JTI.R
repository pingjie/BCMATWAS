...
targetGene <- args[1] ### test: ENSG00000205464.11

targetTissue <- "Breast_Mammary_Tissue"
...
### paths
dhsDir <- "/nobackup/sbcs/pingj2/TWAS/similarity_matrices/DHS/"

allexpDir <- "/nobackup/sbcs/pingj2/data/exp/GTEx/EUR_Female/PEER"

plink_path <- 'plink'

SNPs_in_sumstat <- "/nobackup/sbcs/pingj2/sumstat/breast/EUR/EUR.variants"

genotype_path <- "/nobackup/sbcs/pingj2/EURASN_TWAS/geno/GTEx/GTEx.EUR"

###
workDir <- "/nobackup/sbcs/pingj2/EURASN_TWAS/models/JTI/EUR/GTEx"
setwd(workDir)
...
### 
expSimTab <- fread("/nobackup/sbcs/pingj2/TWAS/similarity_matrices/Exp_Similarity_GTEx_EUR_Female.txt")
...
tissues <- fread("/nobackup/sbcs/pingj2/TWAS/exp/EUR_Female_Tissues_N50.txt", header = F)
...
for (n in 1:nrow(tissues)) {
  tissue <- tissues$V1[n]
  exptab <- loadRData(paste0(allexpDir, "/", tissue, ".GTEx.EUR_Female.PEER_residuals_Inversed.rda"))
...
colnames(dosage_1m) <- c('sampleid', gsub('.{2}$', '', sub('.', '', names(dosage_1m)[-1])))
...
if (nrow(weight_file) > 0){
  ...
  extra_res <- t(c(geneid, gencode$genename[which(gencode$geneid_full == targetGene)], r_test, r_test^2, length(unique(weight_file$rsid)), p_test))
