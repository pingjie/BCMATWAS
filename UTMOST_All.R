...
targetGene <- args[1] # ENSG00000179750.15
...
genesymbol <- gencode$genename[which(gencode$geneid == geneid)]

cat('INFO Starting UTMOST for gene', genesymbol, targetGene, 'at MOD', mod, 'with', ntune, 'tuning parameters and', fold, 'folds', '\n')
...
genotype_path <- "/nobackup/sbcs/pingj2/EURASN_TWAS/geno/allraces"
SNPs_in_sumstat <- "/nobackup/sbcs/pingj2/sumstat/breast/all3races_common.variants"

###
workDir <- "/nobackup/sbcs/pingj2/EURASN_TWAS/models/UTMOST/all/"
setwd(workDir)
...
expdir <- "/nobackup/sbcs/pingj2/EURASN_TWAS/exp/"

projIDs <- c("ABCC.ASN", "Komen.ASN", "Komen.EUR", "GTEx.EUR", "Komen.AFR")
resources <- sapply(projIDs, function(x) strsplit(x, "\\.")[[1]][1])

exp <- list()
for (n in 1:length(projIDs)) {
  projID <- projIDs[n]
  print(projID)
  exptab <- loadRData(paste0(expdir, "/", resources[n], "/", projID, ".PEER_residuals_Inversed.rda"))
...
allsampleIDs <- exp[[1]]$sampleid
for (n in 2:length(exp)) {
    allsampleIDs <- c(allsampleIDs, exp[[n]]$sampleid)
}
...
for (proj_i in 1:length(expressedProjs)) {
  ...
  weight_df <- data.frame(gene = geneid,
  ...
  if (nrow(weight_df) > 0) {
    ...
    extra_res <- t(c(geneid, genesymbol, cv_r[proj_i], (cv_r[proj_i]) ^ 2, length(unique(weight_df$rsid)), cv_p[proj_i]))
