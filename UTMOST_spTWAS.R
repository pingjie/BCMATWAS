...
gene <- args[1] # chr10:100200780:100202088:ENSG00000213341.10
race <- args[2] ### EUR / ASN

geneid <- strsplit(gene, ":")[[1]][4]
tmpid <- paste0(strsplit(gene, ":")[[1]][1], "_", strsplit(gene, ":")[[1]][2], "_", strsplit(gene, ":")[[1]][3], "_", strsplit(gene, ":")[[1]][4])

idmap <- read.table(paste0("/nobackup/sbcs/pingj2/EURASN_TWAS/scripts/junc_", race, "_utmost/junc_", race, "_UTMOST.map.txt"), sep = "\t", header = T, row.names = 1, as.is = T)
...
gencode_path <- "/nobackup/sbcs/pingj2/db/gencode.v26.GRCh38.genes.txt"
gencode <- read.table(gencode_path, header = T, stringsAsFactors = F)

genesymbol <- gencode$genename[which(gencode$geneid_full == geneid)]

cat('INFO Starting UTMOST for APA', genesymbol, gene, 'in', race, 'at MOD', mod, 'with', ntune, 'tuning parameters and', fold, 'folds', '\n')
...
###
workDir <- paste0("/nobackup/sbcs/pingj2/EURASN_TWAS/models/UTMOST_splicing/", race, "/")
setwd(workDir)
...
### Generate Exp table
cat('INFO loading splicing data for junction', genesymbol, gene, '\n')

expdir <- "/nobackup/sbcs/pingj2/EURASN_TWAS/splicing/"
...
for (n in 1:length(projIDs)) {
  projID <- projIDs[n]
  print(projID)
  exptab <- loadRData(paste0(expdir, "/", projID, "/", projID, ".", race, ".juncs.residuals.rda"))
  if (idmap[gene, projID] %in% colnames(exptab)) {
    tmpExpTab <- as.data.frame(matrix(nrow = nrow(exptab), ncol = 2))
    colnames(tmpExpTab) <- c("sampleid", "exp")
    tmpExpTab[, 1] <- rownames(exptab)
    tmpExpTab[, 2] <- exptab[, idmap[gene, projID]]
...
dist <- 500000
chr <- as.numeric(sub('^...', '', strsplit(gene, ":")[[1]][1]))
pos_from <- as.numeric(strsplit(gene, ":")[[1]][2])
pos_to <- as.numeric(strsplit(gene, ":")[[1]][3])
...
for (proj_i in 1:length(expressedProjs)) {
  ...
  weight_df <- data.frame(gene = gene,
  ...
  if (nrow(weight_df) > 0) {
    ...
    extra_res <- t(c(gene, genesymbol, cv_r[proj_i], (cv_r[proj_i]) ^ 2, length(unique(weight_df$rsid)), cv_p[proj_i]))
...
cat('INFO UTMOST for', genesymbol, gene, 'done \n')
