...
gene <- args[1] ### test: chr12:53381560:53381659:clu_12517_+:ENSG00000185591.9
projID <- args[2]
race <- args[3]

geneid <- strsplit(gene, ":")[[1]][5]
tmpid <- paste0(strsplit(gene, ":")[[1]][1], "_", strsplit(gene, ":")[[1]][2], "_", strsplit(gene, ":")[[1]][3], "_", strsplit(gene, ":")[[1]][5])

plink_path <- 'plink'

SNPs_in_sumstat <- paste0("/nobackup/sbcs/pingj2/sumstat/breast/", race, "/", race, ".variants")

pdxDir <- "/nobackup/sbcs/pingj2/EURASN_TWAS/models/spTWAS"
...
genename <- gencode$genename[which(gencode$geneid_full == geneid)]
...
dir.create(paste0(tmp_folder, '/', tmpid))
tmp_folder <- paste0(tmp_folder,'/', tmpid)
options(warn = 0)
...
dosagecmd <- paste0(plink_path,' --bfile ', genotype_path, ' --extract ', SNPs_in_sumstat, ' --chr ', chr, ' --from-bp ', pos_from_gene, ' --to-bp ', pos_to_gene, ' --recode A --out ', tmp_folder, '/', tmpid)
system(dosagecmd, ignore.stdout = T, ignore.stderr = T)
bedcmd <- paste0(plink_path,' --bfile ', genotype_path, ' --extract ', SNPs_in_sumstat, ' --chr ', chr, ' --from-bp ', pos_from_gene, ' --to-bp ', pos_to_gene, ' --make-bed --out ', tmp_folder, '/', tmpid)
system(bedcmd, ignore.stdout = T, ignore.stderr = T)

# Load dosage file
dosage_from_gene <- try(read.table(paste0(tmp_folder, '/', tmpid, '.raw'), header = T, stringsAsFactors = F))
...
# Load Allele Info
snp_info_from_gene <- read.table(paste0(tmp_folder, '/', tmpid, '.bim'), stringsAsFactors = F)
