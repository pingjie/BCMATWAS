...
cat('INFO Starting UTMOST for gene', genesymbol, targetGene, 'in', race, 'at MOD', mod, 'with', ntune, 'tuning parameters and', fold, 'folds', '\n')
...
###
workDir <- paste0("/nobackup/sbcs/pingj2/EURASN_TWAS/models/UTMOST/", race, "/")
setwd(workDir)
...
for (n in 1:length(projIDs)) {
  projID <- projIDs[n]
  print(projID)
  exptab <- loadRData(paste0(expdir, "/", projID, "/", projID, ".", race, ".PEER_residuals_Inversed.rda"))
